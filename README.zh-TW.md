...
- 🔒 **不用帳號、沒有廣告、不追蹤。**
- 🌐 **手機直接變伺服器,不用另外安裝桌面端 App。** 在電腦的任何瀏覽器輸入手機的位址就能用,不必再設定另一個桌面版(例如 [Flow-Desktop](https://github.com/Flow-Tube/Flow-Desktop))。
...
Flow 的電腦端是 [Flow-Desktop](https://github.com/Flow-Tube/Flow-Desktop)。這是在電腦上用你的片庫的另一種方式:開啟伺服器,在任何瀏覽器打開就能用,不用下載、安裝或更新桌面程式。訂閱、觀看紀錄和稍後觀看都是 App 自己的資料,因為都是手機在提供。

...
- 🔒 **不用帳號、沒有廣告、不追蹤。**
- 🌐 **手機直接變伺服器,補齊電腦端。** [Flow-Desktop](https://github.com/Flow-Tube/Flow-Desktop) 沒有 Bilibili。在電腦的任何瀏覽器輸入手機的位址,兩邊都能看,什麼都不用下載。
...
Flow 的電腦端是 [Flow-Desktop](https://github.com/Flow-Tube/Flow-Desktop),它支援 YouTube 和 YouTube Music,但沒有 Bilibili。本地伺服器補的就是這個缺口:開啟後,電腦上的任何瀏覽器都能從你的片庫看 YouTube 和 Bilibili,不用下載或更新桌面程式。訂閱、觀看紀錄和稍後觀看都是 App 自己的資料,因為都是手機在提供。

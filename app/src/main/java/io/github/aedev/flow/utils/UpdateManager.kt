...
object UpdateManager {
    ...
    suspend fun checkForUpdate(currentVersionName: String): UpdateInfo? =
        try {
            fetchUpdate(currentVersionName)
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
    ...
    /**
     * Like [checkForUpdate], but a failed request throws instead of reading as "no update", so a
     * check the user started can say which of the two happened.
     */
    suspend fun fetchUpdate(currentVersionName: String): UpdateInfo? =
        withContext(Dispatchers.IO) {
            ...
            if (!response.isSuccessful) throw java.io.IOException("Update check failed: HTTP ${response.code}")

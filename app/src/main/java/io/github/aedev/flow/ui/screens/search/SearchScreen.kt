...
fun SearchScreen(
    ...
    val submit: (String) -> Unit = { raw ->
        val text = raw.trim()
        if (text.isNotEmpty()) {
            val resolved = resolvePastedVideoLink(text)
            if (resolved != null) {
                val (videoId, videoServiceId) = resolved
                onVideoClick(sharedVideo(videoId, context.getString(R.string.shared_video), videoServiceId))
            } else {
                state.onSubmit(text)
